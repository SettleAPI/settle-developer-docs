<template>
  <section>
    <h3 :id="'v' + version + '-' + $data.anchorLink">
      <!-- <router-link :to="'#v' + version + '-' + resource" class="header-anchor">#</router-link> -->
      <a :href="'#v' + version + '-' + $data.anchorLink" class="header-anchor"
        >#</a
      >
      <router-link :to="resourceOverviewUrl"
        >v{{ version }}.{{ resource }}</router-link
      >
    </h3>
    <table class="table">
      <thead>
          ...
          <td>
            <span class="noLink">
              <router-link :to="method.url">
                <span :class="['badge small', method.http]">{{
                  method.http
                }}</span>
